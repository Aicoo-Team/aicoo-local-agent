...
export async function selectRuntimeAdapter(
  ...
  if (options.kind === "codex") {
    ...
    if (relationshipPolicyFile) {
      ...
      try {
        ...
        options.log?.(`relationship policy could not be loaded; continuing chat-only: ${String(error)}`);
      }
    }
  ...
  const relationshipPolicyFile = options.relationshipPolicyFile
    ...
    : undefined;
  if (relationshipPolicyFile) {
    // The default path now points at a file the user never created, so a malformed,
    // unreadable or self-contained policy must not stop the bridge from starting:
    // tool access is off either way, and a bridge that refuses to boot is worse than
    // one that boots text-only. Same fail-open-to-chat-only rule as the Codex path.
    try {
