...
**agent 只能读这一个文件夹,别的地方一律拒绝。** 先用示例的:

```bash
cd ~/aicoo-dm-agent/dm-agent && ls demo-workspace
```

想换成自己的,把后面命令里的 `demo-workspace` 换成绝对路径即可。
...
```bash
cd ~/aicoo-dm-agent/dm-agent && node src/cli.js start --peer 对方用户名 --workspace demo-workspace
```
...
对方在 https://www.aicoo.io 打开跟你的对话,发一个需要看文件的问题,比如:

> 你 demo-workspace 里 team-notes.md 写了什么?
...
按 `y` 回车 → 对方几秒内收到答案,带 **Local Agent** 标识,和云端 agent 的回复区分开。
